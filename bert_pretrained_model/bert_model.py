...
def get_embeddings(texts):
    ...
    print(requests.post("http://localhost:" + port_num + "/encode", data=data, headers=headers))
...
if __name__ == "__main__":
    text = input('What information do you want to check? (s to stop): ')
    while text != 's':
        ...
        for it, row in df_results.iterrows():
            ...
            print(row['fact_checker'], 'say it is', row['label'], 'because', row['explanation'])
            print('Here is the link:', row['url_checker'])
            print('\n')
